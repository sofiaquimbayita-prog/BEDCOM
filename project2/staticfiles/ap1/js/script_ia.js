...
async function inicializarVosk() {
    ...
    if (!model) {
        ...
        model = await Vosk.createModel('/static/ap1/js/vosk-model-es/');
    }
}
...
window.escucharVoz = async function() {
    ...
    try {
        ...
        const source = model.audioContext.createMediaStreamSource(stream);
        source.connect(recognizer.processor);

    } catch (error) {
        console.error("Error Vosk:", error);
        alert("Error: " + error.message);
        iaQuery.placeholder = "Error de micro.";
        detenerMicrofono();
    }
};
