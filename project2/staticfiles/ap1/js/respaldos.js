...
let esValido = false;
    ...
    btnCerrar.addEventListener('click', cerrarModal); // X button
    btnCancelar.addEventListener('click', cerrarModal);
    // overlay.addEventListener('click', e => { if (e.target === overlay) cerrarModal(); }); // Disabled to prevent closing on overlay click

function validarTodo() {
        // Validate tipo
        const tipoValido = tipoSeleccionado !== null;
        typeGrid.classList.remove('field-valid', 'field-invalid');
        typeGrid.classList.add(tipoValido ? 'field-valid' : 'field-invalid');

        // Validate modulos
        const modulosVisible = modulosSection.classList.contains('visible');
        const modulosValido = !modulosVisible || [...modulosInputs].some(i => i.checked);
        modulosGrid.classList.remove('field-valid', 'field-invalid');
        if (modulosVisible) {
            modulosGrid.classList.add(modulosValido ? 'field-valid' : 'field-invalid');
        }

        // Validate desc
        const hasProhibidos = backupDesc.value && [...caracteresProhibidos].some(char => backupDesc.value.includes(char));
        descSection.classList.remove('field-valid', 'field-invalid');
        if (backupDesc.value.trim()) {
            descSection.classList.add(hasProhibidos ? 'field-invalid' : 'field-valid');
        }

        esValido = tipoValido && modulosValido && !hasProhibidos;
...
function resetModal() {
        ...
        backupDesc.value = '';
        backupDesc.classList.remove('input-error');
        document.getElementById('desc-error').classList.remove('show');
        ...
        actualizarResumen();
        validarTodo(); // Revalidate
    }

    // VALIDACIÓN CARACTERES ESPECIALES DESCRIPCIÓN
    const descError = document.getElementById('desc-error');
    const caracteresProhibidos = "`'+{}[]!\"#$%><&/()=?¡¿*°ªº";

// Toast notification function (igual que otros módulos)
function mostrarNotificacion(titulo, mensaje, tipo) {
    const container = document.getElementById('toast-container-respaldos') || crearToastContainer('respaldos');
    
    const tipoClase = tipo === 'success' ? 'success' : tipo === 'error' ? 'error' : 'info';
    const iconColor = tipo === 'success' ? '#10b981' : tipo === 'error' ? '#ef4444' : '#38bdf8';
...
function crearToastContainer(sufijo) {
    const container = document.createElement('div');
    container.id = `toast-container-${sufijo}`;
    Object.assign(container.style, {
        position: 'fixed', top: '85px', right: '25px', zIndex: '10000',
...
// FUNCIÓN PARA PREVENIR CARACTERES ESPECIALES (IGUAL OTROS MÓDULOS)
backupDesc.addEventListener('keypress', function(e) {
    const char = String.fromCharCode(e.which || e.keyCode);
    if (caracteresProhibidos.includes(char)) {
        e.preventDefault();
        mostrarNotificacionGlobal('¡Carácter bloqueado!', 'Caracteres especiales no permitidos en descripción', 'error');
        return false;
    }
});

backupDesc.addEventListener('input', function() {
        const value = this.value;
        const hasProhibidos = [...caracteresProhibidos].some(char => value.includes(char));
        
        if (hasProhibidos) {
            this.classList.add('input-error');
            descError.classList.add('show');
            descSection.classList.add('field-invalid');
            descSection.classList.remove('field-valid');
        } else {
            this.classList.remove('input-error');
            descError.classList.remove('show');
            if (backupDesc.value.trim()) {
                descSection.classList.add('field-valid');
                descSection.classList.remove('field-invalid');
            }
        }
        
        // Max length visual
        this.style.background = value.length > 450 ? 'rgba(248,113,113,0.1)' : '';
        validarTodo();
    });
    ...
    btnConfirmar.addEventListener('click', () => {
        if (!esValido) {
            mostrarNotificacionGlobal('¡Complete los campos!', 'Seleccione tipo de respaldo, módulos (si parcial) y verifique descripción.', 'error');
            return;
        }
        ...
        mostrarNotificacionGlobal('¡Respaldo creado!', 'Nuevo respaldo agregado al historial correctamente.', 'success');
        actualizarStats();
        cerrarModal();
    });
