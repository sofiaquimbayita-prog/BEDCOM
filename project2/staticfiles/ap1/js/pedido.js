...
$(document).ready(function () {
  ...
  // Filtro DataTables para pedidos anulados
  // Switch APAGADO → solo no anulados | Switch ENCENDIDO → solo anulados
  $.fn.dataTable.ext.search.push(
    function (settings, data, dataIndex) {
      if (settings.nTable.id !== 'tablaPedidos') return true;
      const row = dt.row(dataIndex).node();
      if (!row) return true;
      const val = $(row).data('anulado');
      const anulado = val === true || val === 'true';
      const mostrandoAnulados = $('#toggleAnulados').is(':checked');
      // Si mostrandoAnulados: solo mostrar anulados
      // Si !mostrandoAnulados: solo mostrar NO anulados
      return anulado === mostrandoAnulados;
    }
  );
